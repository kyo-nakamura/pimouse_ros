...
class Motor():
  ...
  def set_raw_freq(self,left_hz,right_hz):
    if not self.is_on:
      rospy.logerr("now enpowered")
      return

    try:
      with  open("/dev/rtmotor_raw_l0","w") as lf, open("/dev/rtmotor_raw_r0","w") as rf:
        lf.wirte(str(int(round(left_hz))) + "\n")
        rf.wirte(str(int(round(right_hz))) + "\n")
    except:
        rospy.logger("cannot write to rtmotor_raw_*")

  def callback_raw_freq1(self, message):
    self.set_raw_freq(messge.left_hz,message.right_hz)

  def callback_cmd_vel(self,message):
    forward_hz = 80000.0*message.liniear.x/(9*math.pi)
